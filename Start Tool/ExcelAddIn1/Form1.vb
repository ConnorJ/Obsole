    ...
    Private Sub CalculateBTN_Click(sender As Object, e As EventArgs) Handles CalculateBTN.Click
        ...
        If IsNumeric(TextBox2.Text) Then
            If TextBox2.Text > 0 Then
                If TextBox2.Text < 100 Then
                    Confidence = CSng(TextBox2.Text) / 100
                Else
                    K = K + "Confidence is larger than One Hundred." & vbNewLine
                End If
            Else
        ...
        If IsNumeric(TextBox16.Text) Then
            If TextBox16.Text >= 0 Then
                If TextBox16.Text <= 100 Then
                    Buffer = CSng(TextBox16.Text) / 100
                Else
                    K = K + "Buffer is larger than One Hundred." & vbNewLine
                End If
            Else
        ...
        Dim Percent_MANF_LOSS As Single
        If IsNumeric(TextBox17.Text) Then
            If TextBox17.Text >= 0 Then
                If TextBox17.Text <= 100 Then
                    Percent_MANF_LOSS = 1 - (CSng(TextBox17.Text) / 100)
                Else
                    K = K + "Percent of Parts Loss in Manufacturing is larger than One Hundred." & vbNewLine
                End If
            Else
                K = K + "Percent of Parts Loss in Manufacturing is smaller than Zero." & vbNewLine
            End If
        Else
            K = K + "Percent of Parts Loss in Manufacturing is not a number." & vbNewLine
        End If
        ...
        If K = "Error: " & vbNewLine Then
            ...
            For i = 1 To Samples
                ...
                If Int_Buy_Length < ComboBox4.Text Then
                    ...
                    Demand = Demand_Sampler(j, Demand_Dist(j), Demand_Mode(j), Demand_Stdev(j), Demand_Low(j), Demand_High(j), Demand_Corr(j))
                End If
                Buy_total(i) = (Total_Demand + ((Length_Of_Buy - Length_Of_Redesign_Mean + Length_Of_Redesign) - Int_Buy_Length) * Demand) / Percent_MANF_LOSS
                Sum = Sum + Buy_total(i)
            Next
            ...
            For j = 1 To Bin_number
                ...
                Charts_Form.Chart2.Series("Total Buy Quanties").Points.AddXY(Bin_name, Bin_Quat(j))
            Next

            '''''Charts_Form.Label1.Text = Math.Round(Mean_value)
            Charts_Form.Label23.Text = Math.Round(Confidence_Value)
            ...
            Charts_Form.Label21.Text = Math.Round(Buffer_percent_modes * 100, 2) & " %"
            Charts_Form.TextBox1.Text = Buffer * 100
